...
msg.TheRank = 'بابا' 
msg.Rank = 1
elseif msg.sender_user_id_ == 1104399382 then 
msg.TheRankCmd = 'Dark' 
msg.TheRank = 'Dark' 
msg.Rank = 1
...
SUDO_USER = redis:hgetall(max..'username:'..SUDO_ID).username
sendPhoto(msg.chat_id_,msg.id_,redis:get(max..':WELCOME_BOT'),[[-› اهلين انا ]]..redis:get(max..':NameBot:')..[[ 🧚‍♀️
-› عشان تفعلني سوي الخطوات هذي 
1- ارفعني مشرفه في المجموعة.
2- حول سجل المحادثة من مخفي إلى ظاهر.
3- أرسل تفعيل في المجموعة وراح اتفعل.
...
msg.text = Mohammad:gsub(Mohammad,mmdi)
	...
	return sendMsg(msg.chat_id_,msg.id_," -› ابشر تم التحديث  ",nil,function(arg,data)
	Refresh_Start = true
	end)
	end 
